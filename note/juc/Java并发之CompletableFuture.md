...
![](http://img.bcoder.top/2020.01.28.2/1.png)
...
上面是代码实现，你先略过 runAsync()、supplyAsync()、thenCombine() 这些不太熟悉的方法，从大局上看，你会发现：
...
![](http://img.bcoder.top/2020.01.28.2/2.png)

![](http://img.bcoder.top/2020.01.28.2/3.png)

![](http://img.bcoder.top/2020.01.28.2/4.png)
...
答：这段代码的问题很多，例如没有异常处理、逻辑不严谨等等，不过更需要关注的是：findRuleByJdbc() 这个方法隐藏着一个阻塞式 I/O，这意味着会阻塞调用线程。默认情况下所有的 CompletableFuture 共享一个 ForkJoinPool，当有阻塞式 I/O 时，可能导致所有的 ForkJoinPool 线程都阻塞，进而影响整个系统的性能。