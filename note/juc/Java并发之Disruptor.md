之前文章介绍过 Java SDK 提供了 2 个有界队列：ArrayBlockingQueue 和 LinkedBlockingQueue，它们都是基于 ReentrantLock 实现的，在高并发场景下，锁的效率并不高，那有没有更好的替代品呢？有，今天我们就介绍一种性能更高的有界队列：Disruptor。
...
2. 能够避免伪共享，提升缓存利用率。
4. 采用无锁算法，避免频繁加锁、解锁的性能消耗。
5. 支持批量消费，消费者可以无锁方式消费多个消息。
...
上面的代码出自官方示例，我略做了一些修改，相较而言，Disruptor 的使用比 Java SDK 提供 BlockingQueue 要复杂一些，但是总体思路还是一致的，其大致情况如下：
...
![](http://img.bcoder.top/2020.01.29.3/1.png)
...
![](http://img.bcoder.top/2020.01.29.3/2.png)
...
![](http://img.bcoder.top/2020.01.29.3/3.png)
...
![](http://img.bcoder.top/2020.01.29.3/4.png)
