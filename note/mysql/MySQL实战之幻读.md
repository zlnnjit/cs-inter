为了便于说明问题，这一篇文章，我们就先使用一个小一点儿的表。建表和初始化语句如下：
...
再来回顾一下上一篇问题，下面的语句序列，是怎么加锁的，加的锁又是什么时候释放的呢？
...
![](http://img.bcoder.top/2020.02.01.8/1.png)
...
![](http://img.bcoder.top/2020.02.01.8/2.png)
...
![](http://img.bcoder.top/2020.02.01.8/3.png)
...
![](http://img.bcoder.top/2020.02.01.8/4.png)
...
![](http://img.bcoder.top/2020.02.01.8/5.png)
...
![](http://img.bcoder.top/2020.02.01.8/6.png)
...
![](http://img.bcoder.top/2020.02.01.8/7.png)
...
可能你会说，这个不是insert … on duplicate key update 就能解决吗？但其实在有多个唯一键的时候，这个方法是不能满足这位提问同学的需求的。
...
![](http://img.bcoder.top/2020.02.01.8/8.png)
...
面文章的评论区有同学留言说，他们公司就使用的是读提交隔离级别加binlog_format=row的组合。他曾问他们公司的DBA说，你为什么要这么配置。DBA直接答复说，因为大家都这么用呀。
...
比如说，大家都用读提交，可是逻辑备份的时候，mysqldump为什么要把备份线程设置成可重复读呢？
...
![](http://img.bcoder.top/2020.02.01.8/9.png)

**1.这里session B和session C的insert 语句都会进入锁等待状态。**

**你可以试着分析一下，出现这种情况的原因是什么？**



session A的select语句加了哪些锁：
...
```sql
begin;
update t set c=5 where id=0;---阻塞了
```

替换成update t set c=11 where id=0;执行不阻塞，**我觉得原因是事务1执行时产生next-key lock范围是(0,5].(5,10]。我想问下update set操作c=xxx是会加锁吗？以及加锁的原理**。

你可以理解为要在索引c上插入一个(c=5,id=0)这一行，是落在(0,5],(5,10]里面的，11可以对吧
