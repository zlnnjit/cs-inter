...
之前文章中，介绍InnoDB的索引结构时，提到了“回表”的概念。我们先来回顾一下这个概念。回表是指，InnoDB在普通索引a上查到主键id的值后，再根据一个个主键id的值到主键索引上去查整行数据的过程。
...
![](http://img.bcoder.top/2020.02.03.8/1.png)
...
![](http://img.bcoder.top/2020.02.03.8/2.png)



![](http://img.bcoder.top/2020.02.03.8/3.png)
...
![](http://img.bcoder.top/2020.02.03.8/4.png)
...
![](http://img.bcoder.top/2020.02.03.8/5.png)
...
我们说到InnoDB的LRU算法的时候提到，由于InnoDB对Bufffer Pool的LRU算法做了优化，即：第一次从磁盘读入内存的数据页，会先放在old区域。如果1秒之后这个数据页不再被访问了，就不会被移动到LRU链表头部，这样对Buffer Pool的命中率影响就不大。
...
![](http://img.bcoder.top/2020.02.03.8/6.png)



![](http://img.bcoder.top/2020.02.03.8/7.png)
...
![](http://img.bcoder.top/2020.02.03.8/8.png)
